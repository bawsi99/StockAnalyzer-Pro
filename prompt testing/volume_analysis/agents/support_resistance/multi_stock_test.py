...
# Add support resistance agent to path
current_dir = os.path.dirname(os.path.abspath(__file__))
sr_agent_path = os.path.join(current_dir, 'src')
sys.path.insert(0, sr_agent_path)

try:
    from support_resistance_agent import SupportResistanceAgent
    from support_resistance_charts import SupportResistanceCharts
except ImportError as e:
    print(f"❌ Import Error for Support Resistance Agent: {e}")
    print("Make sure the support resistance agent is properly installed")
    print(f"SR Agent path: {sr_agent_path}")
    sys.exit(1)
