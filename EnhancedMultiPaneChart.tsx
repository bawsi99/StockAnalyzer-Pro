...
const EnhancedMultiPaneChart: React.FC<EnhancedMultiPaneChartProps> = ({ 
  ...
  const mainPriceSeriesRef = useRef<ISeriesApi<any> | null>(null);
  ...
  function drawRsiDivergenceOverlays({
    rsiDivergences,
    validatedData,
    indicators,
    chartInstance,
    ...
    patternSeriesRef: React.MutableRefObject<{ [key: string]: ISeriesApi<any> }>;
    debug?: boolean;
  }) {
  ...
  }, [showRsiDivergence, rsiDivergences, validatedData, mainPriceSeriesRef.current, chartInstance, rsiInstance, priceChartType]);
      ...
      Object.keys(patternSeriesRef.current).forEach(key => {
        if (key.startsWith('sr_') && chartInstance.current) {
          try {
            ...
            if (debug) console.warn('Failed to remove SR series (cleanup)', key, err);
          }
          delete patternSeriesRef.current[key];
        }
      });
  ...
  useEffect(() => {
    ...
    if (peaksMarker) {
      try {
        ...
        if (debug) console.warn('Failed to remove peaksMarker', e);
      }
    ...
    if (lowsMarker) {
      try {
        ...
        if (debug) console.warn('Failed to remove lowsMarker', e);
      }
